...
sSmartData::sSmartData(U8 *val, U8 *thr, eVendorCode code)
{
    reset();
    setVendorCode(code);
    setSmartBuffer(val, thr);
}

void sSmartData::reset()
{
    setVendorCode(eUnknownVendor);
    setSmartBuffer(NULL, NULL);
}

void sSmartData::setVendorCode(eVendorCode code)
{
    VendorCode = code;
}

void sSmartData::setSmartBuffer(const U8 *val, const U8 *thr)
{
    ValueSector = val;
    ThresholdSector = thr;
}
...
bool SmartUtil::ParseSmartAttribute(const unsigned char* valSector, const tThresholdMap& thresholdMap, sAttribute& attr)
{
    ...
    if(0 == attrID) return false;

    // Search threshold
    unsigned char attrThr = 0;

    tThresholdMap::const_iterator iter;
    iter = thresholdMap.find(attrID);

    if(iter != thresholdMap.end())
    {
        attrThr = iter->second;
    }
...
void SmartUtil::ParseSmartThreshold(const unsigned char* thrSector, tThresholdMap& thresholdMap)
{
    if (NULL == thrSector) return;

    const U8* thrPtr = &thrSector[2];
    ...
    for (int i = 0; i < 30; i++)
    {
        if(0 != thrPtr[0])
        {
            thresholdMap[thrPtr[0]] = thrPtr[1];
        }
...
void SmartUtil::ParseSmartData(const sSmartData& smartData, sSmartInfo& smartInfo, bool autoCorrect)
{
    tThresholdMap thresholdMap;
    ParseSmartThreshold(smartData.ThresholdSector, thresholdMap);

    if (NULL != smartData.ValueSector)
    {
        const U8* valPtr = smartData.ValueSector + 2;
    ...
    if (true == autoCorrect) CorrectSmartInfo(smartData, smartInfo);
}
...
void SmartUtil::CorrectSmartInfo(const sSmartData& smartData, sSmartInfo &info)
{
...
bool SmartUtil::CorrectSmartInfo_Realtek(sSmartInfo& info)
{
    tAttributeMap& orgMap = info.AttrMap;
    tAttributeMap  tmpMap;

    #define MAP_ITEM(orgID, newID) { \
        tAttributeMap::iterator iter; \
        iter = orgMap.find(orgID); \
        if(orgMap.end() != iter) \
        { \
            sAttribute attr = iter->second; \
            attr.ID = newID; \
            CoreUtil::LookupAttributeName(attr.ID, attr.Name); \
            tmpMap[attr.ID] = attr; \
        } \
    }

    MAP_ITEM(  1,   1); //   1       Read_CRC_Error_Sector
    MAP_ITEM(  5,   5); //   5   Reallocated_sectors_count
    MAP_ITEM(  9,   9); //   9              Power_On_Hours
    MAP_ITEM( 12,  12); //  12           Power_Cycle_Count
    MAP_ITEM(163, 168); // 163        MAX_PE_Count_by_spec
    MAP_ITEM(164, 167); // 164         Average_Erase_Count
    MAP_ITEM(166, 162); // 166         Number_of_Bad_Block
    MAP_ITEM(168, 199); // 168        SATA_Phy_Error_Count
    MAP_ITEM(171, 181); // 171          Program_Fail_Count
    MAP_ITEM(172, 182); // 172            Erase_Fail_Count
    MAP_ITEM(174, 192); // 174 Unexpected_Power_Loss_Count
    MAP_ITEM(175, 252); // 175             Read_Fail_Count
    MAP_ITEM(181, 251); // 181      Unaligned_Access_Count
    MAP_ITEM(187, 187); // 187   Uncorrectable_Error_Count
    MAP_ITEM(194, 194); // 194         Current_Temperature
    MAP_ITEM(195, 160); // 195  Uncorrectable_Error_Sector
    //MAP_ITEM(196, ); // 196 No name. Disable it.
    //MAP_ITEM(199, ); // 199 No name. Disable it.
    MAP_ITEM(206, 166); // 206         Minimum_Erase_Count
    MAP_ITEM(207, 165); // 207         Maximun_Erase_Count
    MAP_ITEM(231, 248); // 231         Remaining_PE_Cycles
    MAP_ITEM(232, 249); // 232      Remaining_Spare_Blocks
    MAP_ITEM(241, 241); // 241        Total_Written_in_32M
    MAP_ITEM(242, 242); // 242           Total_Read_in_32M
    MAP_ITEM(249, 250); // 249   Total_Written_To_NAND_32M
