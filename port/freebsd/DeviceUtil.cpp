...
bool DeviceUtil::GetNameList(vector<string> &nameList)
{
    ...
    do {
        DeviceUtil::OpenDevice(nameList, 16, "/dev/ad%d", DeviceUtil::DEVNAME_NUMBER);
        if (0 != nameList.size()) break;

        DeviceUtil::OpenDevice(nameList, 16, "/dev/da%d", DeviceUtil::DEVNAME_NUMBER);
        if (0 != nameList.size()) break;
...
{
    handle = open(deviceName.c_str(), O_RDONLY);
