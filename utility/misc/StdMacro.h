...
#define ALLOC_CHILD(type, child, parent) child = new type(parent)

// GUI Macros -------------------------------------------------------------------------------
#define FRMT_LAYOUT(layout, sp, mr) do { layout->setSpacing(sp); layout->setMargin(mr); } while(0)
#define ALLOC_LAYOUT(ltype, name, spacing, margin) ALLOC_PTR(ltype, name); FRMT_LAYOUT(name, spacing, margin);
#define ALLOC_HBOX(name, spacing, margin) ALLOC_LAYOUT(QHBoxLayout, name, spacing, margin)
#define ALLOC_VBOX(name, spacing, margin) ALLOC_LAYOUT(QVBoxLayout, name, spacing, margin)
#define ADD_LABEL(layout, text, font) do { ALLOC_PTR(QLabel, lbl); lbl->setFont(font); lbl->setText(text); layout->addWidget(lbl); } while(0);
