...
string MiscUtil::GetRandTempName(U32 count)
{
    stringstream sstr;
    // for (U32 i = 0, max = MAX2(count, 1); i < max; i++)
    for (U32 i = 0; i < count; i++)
    {
        sstr << FRMT_U32(5, (rand() % 65536));
    }
...
string MiscUtil::GetRandTempFile()
{
    ...
    stringstream sstr;
    sstr << name << "_" << GetRandTempName() << ".TMP";
...
{
    if (false == SystemUtil::IsAdministrator())
    {
        cout << "Please run program as administrator" << endl;
        exit(1);
    }
}
