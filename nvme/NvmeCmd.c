...
static void CopySmartLog(struct nvme_smart_log smart_log, smart_log_t *smart_des)
{
	int i;

	smart_des->smartraw = smart_log;
	smart_des->critical_warning = smart_log.critical_warning;
	for(i = 0; i < 2; i++)
	{
		smart_des->temperature[i] = smart_log.temperature[i];
	}
	smart_des->avail_spare = smart_log.avail_spare;
	smart_des->spare_thresh = smart_log.spare_thresh;
	smart_des->percent_used = smart_log.percent_used;
	for(i = 0; i < 26; i++)
	{
		smart_des->rsvd6[i] = smart_log.rsvd6[i];
	}
	smart_des->data_units_read = int128_to_double(smart_log.data_units_read);
	smart_des->data_units_written = int128_to_double(smart_log.data_units_written);
	smart_des->host_reads = int128_to_double(smart_log.host_reads);
	smart_des->host_writes = int128_to_double(smart_log.host_writes);
	smart_des->ctrl_busy_time = int128_to_double(smart_log.ctrl_busy_time);
	smart_des->power_cycles = int128_to_double(smart_log.power_cycles);
	smart_des->power_on_hours = int128_to_double(smart_log.power_on_hours);
	smart_des->unsafe_shutdowns = int128_to_double(smart_log.unsafe_shutdowns);
	smart_des->media_errors = int128_to_double(smart_log.media_errors);
	smart_des->num_err_log_entries = int128_to_double(smart_log.num_err_log_entries);
	smart_des->warning_temp_time = le32_to_cpu(smart_log.warning_temp_time);
	smart_des->critical_comp_time = le32_to_cpu(smart_log.critical_comp_time);
	for(i = 0; i < 8; i++)
	{
		smart_des->temp_sensor[i] = le16_to_cpu(smart_log.temp_sensor[i]);
	}
	smart_des->thm_temp1_trans_count = le32_to_cpu(smart_log.thm_temp1_trans_count);
	smart_des->thm_temp2_trans_count = le32_to_cpu(smart_log.thm_temp2_trans_count);
	smart_des->thm_temp1_total_time = le32_to_cpu(smart_log.thm_temp1_total_time);
	smart_des->thm_temp2_total_time = le32_to_cpu(smart_log.thm_temp2_total_time);
	for(i = 0; i < 280; i++)
	{
		smart_des->rsvd232[i] = smart_log.rsvd232[i];
	}
}
...
smart_log_t GetSmartLog(int argc, char **argv)
{
	...
	err = nvme_smart_log(fd, cfg.namespace_id, &smart_log);
	if (!err) {
		CopySmartLog(smart_log, &smart_ret);
		smart_ret.smartraw = smart_log;
		smart_ret.err = 0; //none error
		smart_ret.id = cfg.namespace_id;
		smart_ret.dvname = devicename;
	}
...
device_list_t ListDevice(int argc, char **argv)
{
	...
	for (i = 0; i < n; i++)
		free(devices[i]);
	free(devices);
	// return this list so we don't need to free it, remember to free it after use
	// free(list_items);

	device_lst.lst = list_items;
	device_lst.cnt = list_cnt;
...
{
	// const char *desc = "Retrieve vendor specific log for the given device "
			// "(or optionally a namespace) in either decoded format "
			// "(default) or binary.";
	// const char *namespace = "(optional) desired namespace";
	const char *raw = "output in binary format";
	...
	err = nvme_vendor_specific_log(fd, cfg.namespace_id, ret.data, 512);
	if (!err) {
		ret.err = 0;
		ret.nandwrite = int128_to_double(ret.data);
	}
...
{
	const char *desc = "Retrieve firmware slot information for the given device "\
			"(or optionally a namespace) in either decoded format "\
			"(default) or binary.";
	const char *namespace = "(optional) desired namespace";
	const char *raw = "output in binary format";
